...
@IBDesignable open class RecordView: NSView {
    ...
    // MARK: - Properties
    @IBInspectable open var backgroundColor: NSColor = NSColor.white {
        didSet { needsDisplay = true }
    }
    @IBInspectable open var tintColor: NSColor = NSColor.blue {
        didSet { needsDisplay = true }
    }
    @IBInspectable open var borderColor: NSColor = NSColor.white {
    ...
    fileprivate let clearNormalImage = Util.bundleImage("clear-off")
    fileprivate let clearAlternateImage = Util.bundleImage("clear-on")
...
private extension RecordView {
    func modifierTextAttributes(_ modifiers: NSEventModifierFlags, checkModifier: NSEventModifierFlags) -> [String: AnyObject] {
    ...
    func keyCodeTextAttributes() -> [String: AnyObject] {
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineBreakMode = NSLineBreakMode.byTruncatingTail
        paragraphStyle.baseWritingDirection = NSWritingDirection.leftToRight
        return [NSFontAttributeName: NSFont.systemFont(ofSize: floor(fontSize)),
...
extension RecordView {
    ...
    open override func flagsChanged(with theEvent: NSEvent) {
        if recording {
            ...
            let altTapped = inputModifiers.contains(.option)
            ...
            if (doubleTapModifier.contains(.command) && commandTapped) ||
                ...
                (doubleTapModifier.contains(.option) && altTapped) {
